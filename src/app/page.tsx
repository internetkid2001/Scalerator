...
import { TUNINGS, SCALE_FORMULAS, getScalePositions, CHROMATIC, parseCustomTuning, parseCustomIntervals, getNoteWithOctave } from "@/lib/scales";
...
export default function Home() {
  ...
  const intervals = useMemo(() => {
    ...
    if (scaleName === "Custom") {
      try {
        ...
        setScaleError(e instanceof Error ? e.message : 'Unknown error');
  ...
  const currentTuningDefinition = useMemo(() => {
    ...
    if (tuningName === "Custom") {
      try {
        ...
        setTuningError(e instanceof Error ? e.message : 'Unknown error');
  ...
  const allPositions = useMemo(
    () => {
      ...
      try {
        ...
        console.error("Error computing scale positions:", e instanceof Error ? e.message : e);
  ...
  const playVisibleNotes = useCallback(async () => {
    ...
    .sort((a, b) => {
      const getMidiNumber = (noteStr: string): number => {
    ...
    for (const note of uniqueVisibleNotesToPlay) {
      await playNote(note, 0.3);
      await new Promise(resolve => setTimeout(resolve, 350));
    }
  }, [allPositions, startFret, visibleFrets, currentTuningDefinition]);


  // Calculate draggable bounds for the viewport controller
  const handleDrag = useCallback((_e: unknown, ui: { x: number }) => {
